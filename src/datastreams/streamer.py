...
class Streamer:
    ...
    def runQuery(self, query_field: FieldPath) -> DataFrame:
        ...
        :param dict where: where is a dictionary that specifies query searches. Default is None.
        ...
        # get the values to the right of the last / in the self.endpoint
        endpoint = self.endpoint.split('/')[-1]
        df['endpoint'] = endpoint
        return df

    def runStreamerLoop(self, query_field_list: list[FieldPath], where=None) -> list[DataFrame]:
        """
        ...
        for i in range(len(query_field_list)):
            try:
                df = self.runQuery(query_field_list[i])
                df_data.append(df)
...
# 2/15/23 DISABLED - Unsure if this is needed or if this is needed or if this needs to be deleted.
    # def runSameQuerySearch(self, fieldParam: FieldPath, keys: list[str], values: list, searchKey: str, searchVals: list) -> list[DataFrame]:
    #     """
    ...
    #         df = self.runQuery(fieldParam, where=search_dict)
    #         df_data.append(df)
    #         print(val, df)

    #     return df_data

# 2/5/23 DISABLED - NEED TO REWORK PARALLELIZATION FRAMEWORK. DOESN'T ADD ENOUGH EFFICIENCY. NEEDS REFACTORING
